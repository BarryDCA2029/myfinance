...
function migrate(raw){
  ...
  merged.assets=(Array.isArray(raw.assets)?raw.assets:[]).map(a=>({...a,cost:Number(a.cost??a.value??0),units:Number(a.units||0),price:Number(a.price||0),liquid:a.liquid!==false,liquidity:a.liquidity||((a.kind==='cash')?'ready':(a.kind==='stock'||a.kind==='gold')?'limited':'low'),investmentStatus:a.investmentStatus||'holding',openingAsset:!!a.openingAsset,investmentGroup:a.investmentGroup||'',fundingSource:a.fundingSource||'',saleProceeds:Number(a.saleProceeds||0),realizedPL:Number(a.realizedPL||0)}));
