...
const defaultData={
  ...
  auditLog:[],
  verifiedEmptyDays:[],
  settings:{currency:'THB',hideZeroDebt:true,defaultExpenseAssetId:'',emergencyAssetIds:[],projectOpening:{'ป่ายาง':{invested:40000,returned:11000,note:'รั้ว 30,000 + ปลูก/ดูแล 10,000 · เงินสนับสนุนปีแรก 6,400 + ปีสอง 4,600'}}}
};
...
function projectIcon(n){return ({'บ้าน กทม.':'🏙️','บ้าน เกษตรวิสัย':'🏡','เลี้ยงไก่':'🐓','ป่ายาง':'🌳','ป่ายูคา':'🌳','Polar Farm 1':'🌾','Polar Farm 2':'🌾','ส่วนตัว/ทั่วไป':'👤'})[n]||'◆'}
...
function assets(){
  ...
  return `${header('Assets','ทรัพย์สินและฐานะสุทธิ')}<div class="title-row"><h2 class="page-title">My Wealth</h2><small>Net Worth ${THB(t.netWorth)}</small></div><div class="asset-grid">${kinds.map(([k,n,ic])=>{const list=data.assets.filter(a=>a.kind===k);const v=list.reduce((s,a)=>s+Number(a.value||0),0);return `<button class="asset-card asset-card-btn ${assetFilter===k?'selected':''}" data-asset-kind="${k}"><div class="a-label">${ic} ${n}</div><div class="a-value">${THB(v)}</div><div class="a-sub">${list.length} รายการ · แตะเพื่อดู</div></button>`}).join('')}</div>${assetFilter!=='all'?`<button class="secondary asset-back" id="assetBack">← ดูทรัพย์สินทั้งหมด</button>`:''}${investBox}${t.debtTotal>0?`<section class="section"><div class="card debt-card"><small>หนี้สินรวม</small><b>${THB(t.debtTotal)}</b></div></section>`:''}<section class="section"><div class="section-title"><h2>${esc(title)}</h2><span id="addAsset">+ เพิ่ม</span></div><div class="card tx-list">${filtered.length?filtered.map(a=>{const trackPL=['stock','gold','property','vehicle','other'].includes(a.kind);const pl=trackPL?Number(a.value||0)-Number(a.cost||0)-Number(a.improvementCost||0):0;return `<button class="tx tx-button asset-row" data-asset-id="${a.id}"><div class="tx-ico premium-asset-ico">${assetIcon(a)}</div><div class="tx-main"><b>${esc(a.name)}</b><small>${esc(assetKindLabel(a.kind))}${trackPL&&Number(a.cost||0)>0?` · P/L <span class="${pl>=0?'pos':'neg'}">${pl>=0?'+':''}${THB(pl)}</span>`:''}</small></div><div class="amt ${a.kind==='debt'?'neg':''}">${THB(a.value)}</div></button>`}).join(''):'<div class="empty">ยังไม่มีรายการในหมวดนี้</div>'}</div></section>`
}
function plan(){
  const budgets=currentBudgetMap(); const sums=monthlyExpenseByCategory();
...
function projectLifetime(name){
  ...
  const operatingExpense=round2(expense);
  ...
  return {invested:capital,capital,grossReturned,returned:netReturned,operatingExpense,remaining,pct,note:opening.note||''};
}
...
function projectDetailSheet(){
  ...
  return `<div class="sheet-back" id="sheetBack"><div class="sheet"><div class="grab"></div><h3>${projectIcon(name)} ${esc(name)}</h3><div class="project-kpis"><div><small>เงินลงทุน</small><b>${THB(life.capital)}</b></div><div><small>รับกลับสุทธิ</small><b>${THB(life.returned)}</b></div><div><small>คืนทุน</small><b>${life.pct.toFixed(1)}%</b></div><div><small>ยังไม่คืน</small><b>${THB(life.remaining)}</b></div></div><div class="project-auto"><small>แยกเงินลงทุนออกจากค่าใช้จ่ายดำเนินงาน</small><div>รับกลับรวม ${THB(life.grossReturned)} · ค่าใช้จ่ายดำเนินงาน ${THB(life.operatingExpense)} · รับกลับสุทธิ ${THB(life.returned)}</div></div><div class="notice project-note"><b>เงินลงทุน ≠ ค่าใช้จ่ายดำเนินงาน</b><br>เงินลงทุนตั้งต้นใช้ติดตามการคืนทุนและไม่กระทบ Dashboard ส่วนค่าน้ำ ค่าไฟ ซ่อมแซม และค่าใช้จ่ายประจำที่บันทึกเป็น Expense จะแสดงแยกเป็นค่าใช้จ่ายดำเนินงาน</div><form id="projectDetailForm"><div class="field"><label>เงินลงทุนในทรัพย์สิน / เงินลงทุนตั้งต้น</label><input id="projectOpeningInvested" class="amount-input money-input" type="text" inputmode="decimal" autocomplete="off" value="${Number(opening.invested||0)?num(opening.invested):''}" placeholder="0.00"></div><div class="field"><label>เงินรับกลับก่อนเริ่มใช้แอป</label><input id="projectOpeningReturned" class="amount-input money-input" type="text" inputmode="decimal" autocomplete="off" value="${Number(opening.returned||0)?num(opening.returned):''}" placeholder="0.00"></div><div class="field"><label>รายละเอียด / ที่มาของยอดตั้งต้น</label><textarea id="projectOpeningNote" rows="3" placeholder="เช่น รั้ว 30,000 + ค่าปลูก 10,000">${esc(opening.note||'')}</textarea></div><div class="project-auto"><small>รายการที่บันทึกในแอปแล้ว</small><div>รายรับ Project ${THB(actualIncome)} · รายจ่ายสุทธิ ${THB(actualExpense)}</div></div><button class="primary">บันทึกข้อมูล Project</button>${(Number(opening.invested||0)||Number(opening.returned||0)||opening.note)?`<button type="button" class="danger" id="clearProjectOpening">ล้างยอดตั้งต้น</button>`:''}</form></div></div>`;
}
