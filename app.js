...
function migrate(raw){
  ...
  merged.portfolios=Array.isArray(raw.portfolios)?raw.portfolios:[];
  merged.assets=merged.assets.map(a=>({...a,note:String(a.note||'').slice(0,150),portfolioId:String(a.portfolioId||'')}));
...
function portfolioById(id){return (data.portfolios||[]).find(p=>p.id===id)}
function portfolioValue(p){return data.assets.filter(a=>a.portfolioId===p.id).reduce((s,a)=>s+Number(a.value||0),0)}
function portfolioCost(p){return data.assets.filter(a=>a.portfolioId===p.id&&a.kind==='stock').reduce((s,a)=>s+Number(a.cost||0)+Number(a.improvementCost||0),0)}
function portfolioCards(){
  const ps=data.portfolios||[]; if(!ps.length)return '';
  return `<section class="section"><div class="section-title"><h2>Investment Portfolios</h2><span id="addPortfolio">+ Portfolio</span></div>${ps.map(p=>{const members=data.assets.filter(a=>a.portfolioId===p.id);const cash=members.filter(a=>a.kind==='cash').reduce((s,a)=>s+Number(a.value||0),0);const stocks=members.filter(a=>a.kind==='stock');const total=members.reduce((s,a)=>s+Number(a.value||0),0);return `<div class="card portfolio-card"><button class="portfolio-head" data-portfolio-id="${p.id}"><div><b>${esc(p.name)}</b><small>${stocks.length} holdings · Cash ${THB(cash)}</small></div><strong>${THB(total)}</strong></button><div class="portfolio-members">${members.length?members.map(a=>`<button class="portfolio-member" data-asset-id="${a.id}"><span>${a.kind==='cash'?'💵':'📈'} ${esc(a.name)}</span><b>${THB(a.value)}</b></button>`).join(''):'<div class="empty compact">ยังไม่มี Asset ในพอร์ต</div>'}</div></div>`}).join('')}</section>`;
}
function assets(){
  ...
  return `${header('Assets','ทรัพย์สินและฐานะสุทธิ')}${wealthSwitch('assets')}<div class="title-row"><h2 class="page-title">My Wealth</h2><small>Net Worth ${THB(t.netWorth)}</small></div><div class="asset-grid">${kinds.map(([k,n,ic])=>{const list=data.assets.filter(a=>a.kind===k);const v=list.reduce((s,a)=>s+Number(a.value||0),0);return `<button class="asset-card asset-card-btn ${assetFilter===k?'selected':''}" data-asset-kind="${k}"><div class="a-label">${ic.includes('.')?`<img class="kind-icon" src="${ic}" alt="">`:`<span class="kind-icon-fallback">${ic}</span>`} ${n}</div><div class="a-value">${THB(v)}</div><div class="a-sub">${list.length} รายการ · แตะเพื่อดู</div></button>`}).join('')}</div>${portfolioCards()}${assetFilter!=='all'?`<button class="secondary asset-back" id="assetBack">‹ Assets</button>`:''}${investBox}${t.debtTotal>0?`<section class="section"><div class="card debt-card"><small>หนี้สินรวม</small><b>${THB(t.debtTotal)}</b></div></section>`:''}<section class="section"><div class="section-title"><h2>${esc(title)}</h2><span id="addAsset">+ เพิ่ม</span></div><div class="card tx-list">${filtered.length?filtered.map((a,idx)=>{const trackPL=['stock','gold','property','vehicle','other'].includes(a.kind);const pl=trackPL?Number(a.value||0)-Number(a.cost||0)-Number(a.improvementCost||0):0;return `<button class="tx tx-button asset-row asset-stripe-${idx%2}" data-asset-id="${a.id}"><div class="tx-ico premium-asset-ico">${assetIcon(a)}</div><div class="tx-main"><b>${esc(a.name)}</b><small>${esc(assetKindLabel(a.kind))}${a.portfolioId&&portfolioById(a.portfolioId)?` · ${esc(portfolioById(a.portfolioId).name)}`:''}${a.kind==='stock'&&(/กบข|gpf|ltf/i.test(a.name||''))?' · Long-term Portfolio':''}${trackPL&&Number(a.cost||0)>0?` · P/L <span class="${pl>=0?'pos':'neg'}">${pl>=0?'+':''}${THB(pl)}</span>`:''}</small></div><div class="amt ${a.kind==='debt'?'neg':''}">${THB(a.value)}</div></button>`}).join(''):'<div class="empty">ยังไม่มีรายการในหมวดนี้</div>'}</div></section>`
}
function plan(){
  const budgets=currentBudgetMap(); const sums=monthlyExpenseByCategory();
...
function sheet(){
  ...
  if(modal==='portfolio'||modal==='editPortfolio')return portfolioSheet();
...
function portfolioSheet(){
  const editing=modal==='editPortfolio'; const p=editing?portfolioById(editId):null;
  return `<div class="sheet-back" id="sheetBack"><div class="sheet"><button type="button" class="sheet-nav-back" data-sheet-close>‹ Back</button><h3>${editing?'แก้ไข Portfolio':'เพิ่ม Portfolio'}</h3><p class="field-hint">Portfolio เป็นกลุ่มเท่านั้น ไม่บวกมูลค่าซ้ำ · มูลค่ารวมมาจาก Cash in Port + หุ้นที่อยู่ข้างใน</p><form id="portfolioForm"><div class="field"><label>ชื่อ Portfolio</label><input id="portfolioName" value="${esc(p?.name||'')}" placeholder="เช่น KBank Securities" required></div><div class="field"><label>หมายเหตุ</label><input id="portfolioNote" value="${esc(p?.note||'')}" placeholder="เช่น พอร์ตหุ้นไทย"></div><button class="primary">บันทึก</button>${editing?`<button type="button" class="danger" id="deletePortfolio">ลบกลุ่ม Portfolio</button>`:''}</form></div></div>`;
}
function assetSheet(){
...
function bind(){
  ...
  $('#addPortfolio')?.addEventListener('click',()=>{modal='portfolio';editId=null;render()});
  $$('[data-portfolio-id]').forEach(b=>b.addEventListener('click',()=>{editId=b.dataset.portfolioId;modal='editPortfolio';render()}));
  ...
  $('#assetKind')?.addEventListener('change',e=>{const tracked=['stock','gold','property','vehicle','other'].includes(e.target.value);const sf=$('#trackedAssetFields'),nf=$('#normalValueField');if(sf)sf.style.display=tracked?'block':'none';if(nf)nf.style.display=tracked?'none':'block';const pf=$('#portfolioField');if(pf)pf.style.display=(e.target.value==='cash'||e.target.value==='stock')?'block':'none'});
  $('#assetNote')?.addEventListener('input',e=>{const c=$('#assetNoteCount');if(c)c.textContent=`${e.target.value.length}/150`});
  $('#portfolioForm')?.addEventListener('submit',e=>{e.preventDefault();const name=$('#portfolioName').value.trim();if(!name)return;const row={id:editId||uid(),name,note:($('#portfolioNote')?.value||'').trim().slice(0,150)};if(modal==='editPortfolio'){const i=data.portfolios.findIndex(x=>x.id===editId);if(i>=0)data.portfolios[i]=row}else data.portfolios.push(row);audit(modal==='editPortfolio'?'แก้ไข Portfolio':'เพิ่ม Portfolio',name);save();modal=null;editId=null;render()});
  $('#deletePortfolio')?.addEventListener('click',()=>{const p=portfolioById(editId);if(!p)return;if(confirm('ลบเฉพาะกลุ่ม Portfolio ใช่หรือไม่? Asset และยอดเงินข้างในจะไม่ถูกลบ')){data.assets.forEach(a=>{if(a.portfolioId===p.id)a.portfolioId=''});data.portfolios=data.portfolios.filter(x=>x.id!==p.id);audit('ลบ Portfolio',p.name);save();modal=null;editId=null;render()}});
  $('#assetForm')?.addEventListener('submit',e=>{
    e.preventDefault(); const oldAsset=editId?findAsset(editId):null; const kind=$('#assetKind').value; const tracked=['stock','gold','property','vehicle','other'].includes(kind); const valueEl=tracked?$('#assetValue'):$('#assetValueNormal'); const row={id:editId||uid(),name:$('#assetName').value.trim(),kind,value:parseMoney(valueEl?.value),cost:tracked?parseMoney($('#assetCost')?.value||0):Number(oldAsset?.cost||0),improvementCost:tracked?parseMoney($('#assetImprovementCost')?.value||0):Number(oldAsset?.improvementCost||0),units:kind==='stock'?parseMoney($('#assetUnits')?.value||0):0,price:kind==='stock'?parseMoney($('#assetPrice')?.value||0):0,liquid:oldAsset?.liquid!==false,liquidity:$('#assetLiquidity')?.value||oldAsset?.liquidity||'ready',note:($('#assetNote')?.value||'').trim().slice(0,150),portfolioId:(kind==='cash'||kind==='stock')?($('#assetPortfolio')?.value||''):''};
