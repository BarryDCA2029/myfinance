...
function projectLifetime(name){
  ...
  const invested=round2(Number(opening.invested||0)+expense),returned=round2(Number(opening.returned||0)+income);
  const remaining=Math.max(0,round2(invested-returned)),pct=invested?Math.min(100,returned/invested*100):0;
  return {invested,returned,remaining,pct,note:opening.note||''};
}
